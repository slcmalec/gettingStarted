...
\item Now we initilize the git repository. In the
top level \verb!sampleCourse! directory, we run the following commands.
\begin{verbatim}
git init
git add .
git commit -am "Initializing our new course"
\end{verbatim}
...
\begin{center}
\begin{image}
\includegraphics[scale=.3]{RepoInit}
\end{image}
\end{center}
\end{enumerate}
\end{document}
