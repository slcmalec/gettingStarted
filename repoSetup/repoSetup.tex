...
\begin{enumerate}
\item \label{maketopdir}
Create a directory for your course files
...
We will create a course that has only one activity. Using any text editor,
...
\item Next we create a new directory
called \verb!example! (inside the \verb!sampleCourse! directory)
containing a file called \verb!example.tex!. 
In
the same terminal session from step \ref{maketopdir}
\begin{verbatim}
mkdir example
cd example
touch example.tex
...
\item 
The activity contained in \verb!example.tex!
