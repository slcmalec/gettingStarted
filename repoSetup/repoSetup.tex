...
that they should be presented to students. In the example below
there is one activity file \verb!example.tex!
written without the extension \verb!.tex!
contained in a directory \verb!example!.
Generally courses should have more activities than the
example above. In this case each activity should be listed
in \verb!course.xim! with indent
\begin{verbatim}
---
...
In general, we recommend placing each
...
should use the document class \verb!ximera!
...
\end{document}
\end{verbatim}
The activity at this stage contains no actual content.

\item On \href{http://github.org}{\tt github.org},
create a repository. See the image below.
\begin{image}
...
Upon accepting the default settings, \href{http://github.org}{\tt github.org}
will respond with additional directions:
\begin{verbatim}
touch README.md
git init
git add README.md
...
\item Click on \verb!settings! button on the
