...
These directions assume you have gone through the steps of initializing your git repository in the previous activity.\\


A \link[Ximera]{http://ximera.osu.edu} course consists of a directory (\verb!anExampleCourse!)  containing a set of directories
     (\verb!theFirstActivity!, \verb!theSecondActivity!) that all
     contain \LaTeX\ files using the \verb!ximera! document class. This directory also contains one (or many)  \LaTeX\ files
     using the \verb!xourse! document class. This section describes how to create a course from scratch. 
\begin{enumerate}
...
You will not need to repeat the \verb!git config! lines again, but every time you add a new file, you will need to run \verb!git add! to add it, and every time you change your files you will run \verb!git commit -m "short description of changes"! to commit the changes to the server. The description of changes is necessary to commit, as is the \verb! -m!.
...
\end{remark}
\item Now add this file to git and commit your changes. Back in the terminal (and still in your \verb!theFirstActivity! directory), type 
\begin{verbatim}
...
\item Now we will compile all these files. Type \verb!xake name! followed by a space and then a short lowercase name for your xandbox. This could be your first name, for instance. The name you choose must be globally unique, so be creative! For instance, \verb!xake name turnloon! is what I will choose.

\item Type \verb!xake bake! to compile your first.tex into an html file. Type \verb!xake frost! to create a publication commit on top of your source commit. Finally type \verb!xake serve! to share your content with the world. For instance, my content will appear at https://ximera.osu.edu/turnloon/anExampleCourse
